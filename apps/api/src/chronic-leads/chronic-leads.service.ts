...
export class ChronicLeadsService {
  ...
  async findAll(query: QueryChronicLeadsDto, user: RequestUser) {
    await this.assertLocalLeadPoolUser(user);
  ...
  async findActiveLeadForPatient(criteria: {
    ...
    await this.resolveConfiguredOrSingleLocalHospitalTenant();
  ...
  async contact(id: string, dto: NoteChronicLeadDto, user: RequestUser, ipAddress?: string) {
    ...
    await this.assertLocalLeadPoolUser(user);
    const before = await this.findOneRaw(id);
  ...
  async sign(id: string, dto: SignChronicLeadDto, user: RequestUser, ipAddress?: string) {
    ...
    // Each production installation is hospital-local. Bind newly promoted
    // patients to the configured local hospital (or the operator's hospital)
    // instead of creating tenantless records. This preserves defense-in-depth
    // without disabling the core sign-up workflow.
    ...
    const { lead, createdPatient } = await this.prisma.$transaction(async (tx) => {
      // 1) 建 Patient（如未存在）；旧记录缺少医院归属时在本地部署内安全回填。
  ...
  /**
   * Resolve the hospital for this local installation. ChronicLead does not yet
   * carry hospitalTenantId, so its pool is safe only when the instance is
   * explicitly pinned or has exactly one active hospital. Ambiguous instances
   * fail closed instead of silently assigning or exposing a lead across sites.
   */
  private async resolveConfiguredOrSingleLocalHospitalTenant(): Promise<string> {
    const configuredTenantId = (process.env.LOCAL_HOSPITAL_TENANT_ID ?? '').trim();
    if (configuredTenantId) {
      const configured = await this.prisma.hospitalTenant.findUnique({
        where: { id: configuredTenantId },
        select: { id: true, isActive: true },
      });
      if (!configured || !configured.isActive) {
        throw new ForbiddenException('LOCAL_HOSPITAL_TENANT_ID 未指向启用中的医院');
      }
      return configured.id;
    }

    const activeTenants = await this.prisma.hospitalTenant.findMany({
      where: { isActive: true },
      select: { id: true },
      take: 2,
    });
    if (activeTenants.length === 1) return activeTenants[0].id;

    throw new ForbiddenException(
      '本地数据库存在多个启用医院，无法唯一确定线索归属。请在 apps/api/.env 配置 LOCAL_HOSPITAL_TENANT_ID。',
    );
  }

  private async assertLocalLeadPoolUser(user: RequestUser): Promise<string> {
    const localTenantId = await this.resolveConfiguredOrSingleLocalHospitalTenant();
    if (user.role !== UserRole.ADMIN && user.hospitalTenantId !== localTenantId) {
      throw new ForbiddenException('当前账号不属于本地部署医院，禁止操作高危线索');
    }
