...
export class PatientAppService {
  ...
  async lookupIdentity(dto: IdentityLookupDto, miniCtx?: MiniProgramContext) {
    ...
    // Public identity lookup must not become a one-field patient enumeration
    // oracle. Require phone plus a second hospital-controlled identifier before
    // returning any lead / patient snapshot.
    if (!phone || (!hospitalPatientId && !idCardLast4)) {
      return { matchType: 'NOT_FOUND', reason: 'NEED_STRONGER_IDENTITY_PROOF', ...base };
    }
  ...
  private async resolveSystemOperator(): Promise<RequestUser> {
    ...
    const localHospitalTenantId = (process.env.LOCAL_HOSPITAL_TENANT_ID ?? '').trim();
    for (const role of preferredRoles) {
      const user = await this.prisma.user.findFirst({
        where: {
          role,
          isActive: true,
          ...(localHospitalTenantId ? { hospitalTenantId: localHospitalTenantId } : {}),
        },
  ...
  private async tryLookupHisPatient(
    ...
    try {
      const result = await this.hisIntegrationService.lookupPatientByBarcodeForPatientSelfBind(
        hospitalPatientId,
      );
  ...
  async findBindingRequests(user: RequestUser, status?: string, hospitalTenantId?: string) {
    ...
    const where: Prisma.PatientBindingRequestWhereInput = {
      patientId: { in: visiblePatients.map((patient) => patient.id) },
  ...
  private async assertBindingRequestWritable(user: RequestUser, id: string) {
  ...
  async approveBindingRequest(id: string, user: RequestUser, ipAddress?: string) {
    ...
    await this.assertBindingRequestWritable(user, id);
