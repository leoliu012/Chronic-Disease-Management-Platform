...
export class HospitalVisitRemindersService {
  ...
  private getReminderTaskWhere(reminder: {
    ...
    if (reminder.sourceTaskId) return { id: reminder.sourceTaskId };
    if (reminder.riskEpisodeId) {
      return { riskEpisodeId: reminder.riskEpisodeId, status: { in: [TaskStatus.PENDING, TaskStatus.IN_PROGRESS] } };
    }
    return {
  ...
  private async findOpenReminderTask(reminder: {
    ...
    return this.prisma.task.findFirst({
      where: {
        AND: [
          this.getReminderTaskWhere(reminder),
          { status: { in: [TaskStatus.PENDING, TaskStatus.IN_PROGRESS] } },
        ],
      },
  ...
  private async ensureHospitalVisitTask(
    ...
    if (existingTask) {
      if (reminder.sourceTaskId !== existingTask.id || reminder.riskEpisodeId !== existingTask.riskEpisodeId) {
        await this.prisma.hospitalVisitReminder.update({
          where: { id: reminder.id },
          data: { sourceTaskId: existingTask.id, riskEpisodeId: existingTask.riskEpisodeId ?? reminder.riskEpisodeId ?? undefined },
        });
      }
      return existingTask;
    }
    ...
    const relatedAlert = reminder.sourceRiskAlertId
      ? await this.prisma.riskAlert.findUnique({
          ...
          select: { title: true, status: true, riskEpisodeId: true, riskLevel: true },
        })
      : null;
    if (relatedAlert && CLOSED_ALERT_STATUSES.includes(relatedAlert.status)) return null;

    if (reminder.sourceRiskAlertId) {
      const ensured = await this.disposition.ensureOpenTaskForAlert(this.prisma, {
        alertId: reminder.sourceRiskAlertId,
        taskTitle: relatedAlert?.title ?? '到院提醒处理',
        taskType: HOSPITAL_VISIT_TASK_TYPE,
        dueAt,
        assigneeId: reminder.patient?.responsibleNurseId ?? reminder.remindedBy ?? user?.id,
      });
      await this.prisma.hospitalVisitReminder.update({
        where: { id: reminder.id },
        data: { sourceTaskId: ensured.task.id, riskEpisodeId: ensured.task.riskEpisodeId ?? undefined },
      });
      return ensured.task;
    }
    ...
    const task = await this.prisma.task.create({
      data: {
        ...
        riskEpisodeId: reminder.riskEpisodeId ?? relatedAlert?.riskEpisodeId ?? undefined,
        priority: relatedAlert?.riskLevel === 'VERY_HIGH' ? 0 : relatedAlert?.riskLevel === 'HIGH' ? 1 : 2,
      },
    });
    await this.prisma.hospitalVisitReminder.update({
      where: { id: reminder.id },
      data: { sourceTaskId: task.id, riskEpisodeId: task.riskEpisodeId ?? undefined },
    });
    return task;
  }
  ...
  private async attachRelatedTask<
    T extends { patientId: string; sourceRiskAlertId?: string | null; sourceTaskId?: string | null; riskEpisodeId?: string | null },
  >(reminder: T) {
  ...
  async findAll(user: RequestUser, status?: HospitalVisitReminderStatus, hospitalTenantId?: string) {
    ...
    const reminders = await this.prisma.hospitalVisitReminder.findMany({
      where: { patient: patientScope, ...(status ? { status } : {}) },
  ...
  async create(
    ...
    const sourceAlert = dto.sourceRiskAlertId
      ? await this.prisma.riskAlert.findUnique({ where: { id: dto.sourceRiskAlertId }, include: { riskEpisode: true } })
      : null;
    if (dto.sourceRiskAlertId && (!sourceAlert || sourceAlert.patientId !== patientId)) {
      throw new NotFoundException('Source risk alert not found for this patient');
    }
    const sourceTask = dto.sourceTaskId
      ? await this.prisma.task.findUnique({ where: { id: dto.sourceTaskId } })
      : sourceAlert?.riskEpisode?.openTaskId
        ? await this.prisma.task.findUnique({ where: { id: sourceAlert.riskEpisode.openTaskId } })
        : null;
    if (dto.sourceTaskId && (!sourceTask || sourceTask.patientId !== patientId)) {
      throw new NotFoundException('Source task not found for this patient');
    }
    ...
    const reminder = await this.prisma.hospitalVisitReminder.create({
      data: {
        ...
        riskEpisodeId: sourceAlert?.riskEpisodeId ?? sourceTask?.riskEpisodeId,
        reason,
        note: dto.note,
        remindedBy: user.id,
        status: HospitalVisitReminderStatus.ACTIVE,
    ...
    await this.auditService.record({
      ...
      afterData: {
        ...
        riskEpisodeId: sourceAlert?.riskEpisodeId ?? sourceTask?.riskEpisodeId ?? null,
        reason,
        note: dto.note,
        electronicSignature,
  ...
  private async updateStatus(
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.task.updateMany({
        ...
        data: {
          status: status === HospitalVisitReminderStatus.REVOKED ? TaskStatus.CANCELED : TaskStatus.DONE,
        },
      });
