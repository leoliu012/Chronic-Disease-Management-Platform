...
export class ChronicLeadsService {
   ...
   */
  async sign(_id: string, _dto: SignChronicLeadDto, user: RequestUser, _ipAddress?: string) {
    this.assertRole(user);

    // Fail closed until ChronicLead carries hospitalTenantId (or an explicit,
    // validated source-to-tenant mapping). Promoting an unscoped lead would
    // create an unscoped Patient and defeat the clinical access boundary.
    throw new ForbiddenException(
      '高危线索尚未绑定医院访问域，暂不能升档建档。请先完成 ChronicLead 医院归属迁移。',
    );
  }
