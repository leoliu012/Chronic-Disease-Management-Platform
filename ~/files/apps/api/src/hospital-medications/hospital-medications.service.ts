...
export class HospitalMedicationsService {
  ...
  async create(createDto: CreateHospitalMedicationDto) {
    ...
    if (rest.encounterRecordId) {
      const encounter = await this.prisma.encounterRecord.findUnique({ where: { id: rest.encounterRecordId } });
      if (!encounter || encounter.patientId !== patientId) {
        throw new NotFoundException('Encounter record not found for this patient');
      }
    }
