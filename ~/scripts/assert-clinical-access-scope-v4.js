...
mustContain(
  ...
  '暂不能升档建档',
  'unmapped ChronicLead promotion fail-closed guard',
);
const chronicLeadsController = text(path.join(root, 'apps/api/src/chronic-leads/chronic-leads.controller.ts'));
if (
  chronicLeadsController.includes('UserRole.NURSE') ||
  chronicLeadsController.includes('UserRole.DOCTOR') ||
  chronicLeadsController.includes('UserRole.MANAGER')
) failures.push('apps/api/src/chronic-leads/chronic-leads.controller.ts: ChronicLead HTTP endpoints must fail closed to ADMIN until tenant mapping exists');
...
console.log('[ok] HIS lookup/export are scoped and unmapped global surfaces fail closed');
